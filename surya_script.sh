...
clang() {
    ...
    if [ ! -d "clang" ]; then
        ...
        KBUILD_COMPILER_STRING="Proton clang 17.0 x Raven"
...
LC_ALL=C
export LC_ALL
export token=7156614633:AAEcv-ttSka3mH8wS45usUlpRW4FbqgT8xQ
export chat_id=-1001592026548
...
compile() {
    ...
    git clone --depth=1 https://github.com/farhan-venn/AnyKernel3.git AnyKernel -b Raven
    cp out/arch/arm64/boot/Image.gz-dtb AnyKernel
}
# Zipping
zipping() {
    cd AnyKernel || exit 1
    zip -r9 Raven-Kernel-"${BRANCH}"-"${CODENAME}"-"${DATE}".zip ./*
    cd ..
}
