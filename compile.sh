#!/bin/bash
#
# CTRL + F FOR FIND AND REPLACE XXX TO YOUR DEVICE OR ANYTHING
# 

SECONDS=0 # builtin bash timer
ZIPNAME="RavenKernel$(date '+%Y%m%d-%H%M').zip"
...
export KBUILD_BUILD_USER=Niken
export KBUILD_BUILD_HOST=Farhan
...
if [ -f "$kernel" ] && [ -f "$dtb" ] && [ -f "$dtbo" ]; then
	...
	elif ! git clone -q https://github.com/MobiusPrjkt/AnyKernel3 -b Mobius; then
